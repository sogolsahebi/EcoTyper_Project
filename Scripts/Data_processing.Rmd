...
combined_clin <- combined_clin[combined_clin$study %in% datasets_25_or_more, ]
...
filtered_combined_clin <- combined_clin[combined_clin$rna %in% c("rnaseq", "tpm", "fpkm"), ] # 87 patients is removed
filtered_combined_clin_sex <- combined_clin_sex[combined_clin_sex$rna %in% c("rnaseq", "tpm", "fpkm"), ]


# Save as csv files
#write.csv(combined_clin, file = file.path(Base_dir, "outputs/Dataprocessing_outputs", "combined_clin_after_exclusion.csv"), row.names = TRUE)
...
after_df <- merge(
  generate_all_data(combined_clin), 
  generate_all_data(combined_clin_sex), 
  by = "Characteristic", 
  all = TRUE
)
...
28 datasets remain after exclusion.
...
```{r Pie charts , width = 15, height = 820}
...
```{r Snakey diagram for IO datasets}

library(networkD3)
library(dplyr)

# Calculate total patients
total_patients <- sum(summary_table$Patients, na.rm = TRUE)

# Summarize data for Sankey diagram
summary_sankey <- summary_table %>%
  group_by(Cancer_Type, Treatments, Clinical_Endpoints) %>%
  summarise(Total_Patients = sum(Patients, na.rm = TRUE), .groups = "drop") %>%
  mutate(Response = ifelse(Cancer_Type %in% c("Bladder", "Lung", "Melanoma"), "Responders", "Non-Responders"))
...
# Third layer: Treatments to Responses
links <- links %>%
  bind_rows(
    summary_sankey %>%
      mutate(
        source = match(paste0(Treatments, "\n(n=", 
                              sapply(Treatments, function(t) sum(summary_sankey$Total_Patients[summary_sankey$Treatments == t])), 
                              ")"), nodes$name) - 1,
        target = ifelse(Response == "Responders", match("Responders", nodes$name) - 1, match("Non-Responders", nodes$name) - 1),
        value = Total_Patients
      ) %>%
      select(source, target, value)
  )

# Fourth layer: Treatments to Clinical Endpoints
links <- links %>%
  bind_rows(
    summary_sankey %>%
      mutate(
...
# saveNetwork(sankey, file = file.path("~/BHK lab/EcoTyper_Project//outputs/", "Sankey_Cancer_Treatments_Response_Annotated.html"))
...
```{r Snakey2 diagram for IO datasets}


generate_annotated_sankey <- function(data, output_filename) {

  # Calculate the total number of patients for percentages
  ...
  # Prepare Nodes with percentages and counts
  nodes <- data.frame(
    name = c(
      paste0("Total Patients (n=", total_patients, ")"),
      paste0(unique(summary_sankey$Cancer_Type), " (n=", 
             sapply(unique(summary_sankey$Cancer_Type), function(x) 
               sum(summary_sankey$Total_Patients[summary_sankey$Cancer_Type == x])), 
             ", ", 
             round(sapply(unique(summary_sankey$Cancer_Type), function(x) 
               sum(summary_sankey$Total_Patients[summary_sankey$Cancer_Type == x]) / total_patients * 100), 2), 
             "%)"),
      paste0(unique(summary_sankey$Treatments), " (n=", 
             sapply(unique(summary_sankey$Treatments), function(x) 
               sum(summary_sankey$Total_Patients[summary_sankey$Treatments == x])), 
             ", ", 
             round(sapply(unique(summary_sankey$Treatments), function(x) 
               sum(summary_sankey$Total_Patients[summary_sankey$Treatments == x]) / total_patients * 100), 2), 
             "%)"),
      paste0(c("Responders", "Non-Responders"), " (n=", 
             c(sum(summary_sankey$Total_Patients[summary_sankey$Response == "Responders"]),
               sum(summary_sankey$Total_Patients[summary_sankey$Response == "Non-Responders"])),
             ", ", 
             c(round(sum(summary_sankey$Total_Patients[summary_sankey$Response == "Responders"]) / total_patients * 100, 2),
               round(sum(summary_sankey$Total_Patients[summary_sankey$Response == "Non-Responders"]) / total_patients * 100, 2)),
             "%)")
    )
  )
  ...
  links <- summary_sankey %>%
    mutate(
      source_total = 0, # Link all cancer types to Total Patients node
      source_cancer = match(Cancer_Type, gsub(" \\(.*", "", nodes$name)) - 1,
      target_treatment = match(Treatments, gsub(" \\(.*", "", nodes$name)) - 1,
      target_response = ifelse(Response == "Responders", match("Responders", gsub(" \\(.*", "", nodes$name)) - 1, match("Non-Responders", gsub(" \\(.*", "", nodes$name)) - 1)
    ) %>%
    select(source = source_total, target = source_cancer, value = Total_Patients) %>%
    bind_rows(
      summary_sankey %>%
        mutate(
          source = match(Cancer_Type, gsub(" \\(.*", "", nodes$name)) - 1,
          target = match(Treatments, gsub(" \\(.*", "", nodes$name)) - 1,
          value = Total_Patients
        ) %>%
        select(source, target, value)
    ) %>%
    bind_rows(
      summary_sankey %>%
        mutate(
          source = match(Treatments, gsub(" \\(.*", "", nodes$name)) - 1,
          target = ifelse(Response == "Responders", match("Responders", gsub(" \\(.*", "", nodes$name)) - 1, match("Non-Responders", gsub(" \\(.*", "", nodes$name)) - 1),
          value = Total_Patients
        ) %>%
        select(source, target, value)
    )
...
# Usage
# For summary_table
sankey_beforee <- generate_annotated_sankey(summary_table, "Sankey_Before_Exclusion.html")

# For summary_table_after
sankey_afterr <- generate_annotated_sankey(summary_table_after, "Sankey_After_Exclusion.html")

sankey_beforee
sankey_afterr

# saveNetwork(sankey_beforee, file = "~/BHK lab/EcoTyper_Project/outputs/Dataprocessing_outputs/Plots/2Sankey_Cancer_Treatments_Endpoints_Before.html")
# saveNetwork(sankey_afterr, file = "~/BHK lab/EcoTyper_Project/outputs/Dataprocessing_outputs/Plots/2Sankey_Cancer_Treatments_Endpoints_After2.html")
