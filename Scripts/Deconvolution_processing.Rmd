...
# Print the range of scaled data
  print(paste("Range for", "Cibersort", ":"))
  print(range(scaled_cibersort_results))
...
```{r ESTIMATE for Jung Jung datasets,fig.height=4, fig.width=12}
#5.ESTIMATE
...
# Challenge: Why TIMER Won't Work

# From the TIMER paper, consider these key steps:
# 1. Filter Low or Zero Expression Genes:
#    Remove genes with low or zero expression in a significant number of samples to avoid noise.
# 2. Adjust Expression Data:
#    Use the log2(TPM + 0.001) formula to transform expression data, ensuring proper scaling for TIMER analysis.
# 3. Check for Zero Values and NAs:
#    Ensure that after transformation, there are no zero values or missing data (NAs) left.
# 4. Run TIMER:
#    After confirming data quality, run the TIMER analysis with your adjusted and filtered expression data.
